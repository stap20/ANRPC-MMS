...
/*
        ...
        private void EditBtn_Click(object sender, EventArgs e)
        {
            ...
            if ((MessageBox.Show("هل تريد تعديل الاضافة المخزنية؟", "", MessageBoxButtons.YesNo)) == DialogResult.Yes)
            {
                ...
                {
                    MessageBox.Show("يجب اختيار الاضافة المخزنية المراد تعديله");
                    return;
                }
                else
                {
        ...
        private void SaveBtn_Click(object sender, EventArgs e)
        {
            if (AddEditFlag == 2)
            {
                ...
                foreach(DataGridViewRow row in dataGridView1.Rows)
                {
                    if(!row.IsNewRow)
                    {
                string cmdstring = "exec SP_InsertEdafa @p1,@p2,@p3,@p4,@p44,@p444,@p5,@p6,@p7,@p8,@p9,@p10,@p11,@p12,@p13,@p14,@p15,@p16,@p17,@p18,@p19,@p20,@p21,@p22,@p23,@p233,@p24,@p34 out";
                SqlCommand cmd = new SqlCommand(cmdstring, con);
               ...
               cmd.Parameters.AddWithValue("@p21", DBNull.Value);//shroot
                ...
                cmd.Parameters.AddWithValue("@p233",TXT_MomayzReal.Text.ToString() );
        ...
        public void UpdateEdafa()
        {
           ...
           //   Input_Reset();
                ...
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    if (!row.IsNewRow)
                    {
                      cmdstring = "exec SP_UpdateEdafa @fff,@p1old,@p2old,@p1,@p2,@p3,@p4,@p44,@p444,@p5,@p6,@p7,@p8,@p9,@p10,@p11,@p12,@p13,@p14,@p15,@p16,@p17,@p18,@p19,@p20,@p21,@p22,@p23,@p233,@p34 out,@p35";
                       ...
                       cmd.Parameters.AddWithValue("@fff", FlagSign3);
                        ...
                        cmd.Parameters.AddWithValue("@p22", Constants.User_Name.ToString());
                        cmd.Parameters.AddWithValue("@p23", Convert.ToDateTime(DateTime.Now.ToShortDateString()));
                        cmd.Parameters.AddWithValue("@p23", TXT_MomayzReal.Text.ToString());
                        cmd.Parameters.Add("@p34", SqlDbType.Int, 32);  //-------> output parameter
                        cmd.Parameters["@p34"].Direction = ParameterDirection.Output;
                ...
                if (FlagSign3 == 1)
                {
                    ...
                    if (TXT_MomayzReal.Text.ToString() == "70")/////mobashr
                    {
        ...
        private void Cmb_FY2_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (AddEditFlag == 2)//add
            {
                ...
                // string cmdstring = "Exec SP_getlast @TRNO,@MM,@YYYY,@Num output";
                string cmdstring = "select  ( COALESCE(MAX( Edafa_No), 0))  from  T_Edafa where Edafa_FY=@FY ";
                SqlCommand cmd = new SqlCommand(cmdstring, con);
        ...
        public void SearchEdafa(int x)
        {
            ...
            Constants.opencon();
            // string cmdstring = "Exec SP_getlast @TRNO,@MM,@YYYY,@Num output";
            string cmdstring = "select * from   T_Edafa where Edafa_No=@TN and Edafa_FY=@FY";
            SqlCommand cmd = new SqlCommand(cmdstring, Constants.con);
            if (x == 1)
            {
            ...
            if (dr.HasRows == true)
            {
                while (dr.Read())
                {
                  ...
                  TXT_MomayzReal.Text = dr["MomayzReal"].ToString();
                  if (TXT_MomayzReal.Text.ToString() == "")
                  {

                  }
                  else
                  {
                      Cmb_CType.SelectedValue = TXT_MomayzReal.Text.ToString();
                  }
        ...
        {
            Constants.opencon();
            string cmdstring = "Exec SP_deleteTR2 @TNO,@FY";
            ...
            foreach (DataGridViewRow row in dataGridView1.Rows)
            {
                ...
                if (!row.IsNewRow)
                {
                    if ((row.Cells[25].Value.ToString() == "True"))//lw motabk
                    {
                        ...
                        cmd.Parameters.AddWithValue("@p6", TXT_MomayzReal.Text.ToString());
        ...
        private void Cmb_CType_SelectedIndexChanged(object sender, EventArgs e)
        {
           ...
           // Cmb_CType.SelectedIndexChanged += new EventHandler(Cmb_CType_SelectedIndexChanged);
            ...
            else
            {
                ...
                TXT_MomayzReal.Text = Cmb_CType.SelectedValue.ToString();
            }
