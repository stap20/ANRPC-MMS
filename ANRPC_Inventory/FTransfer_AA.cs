...
{
    public partial class FTransfer_AA : Form
    {
        List<CurrencyInfo> currencies = new List<CurrencyInfo>();
        public SqlConnection con;//sql conn for anrpc_sms db

        public SqlConnection con3;//sql conn for anrpc_sms db
        Image DefaulteImg;
        Image image1;
        Image image2;
        ...
        private void TalbTawred_Load(object sender, EventArgs e)
        {
            ...
            con3 = new SqlConnection(Constants.constring3);
            ...
            cmdstring = "SELECT  [CCode],[CName] FROM [T_TransferTypes] where CType=3 and CFlag=2";//will use cmdstring3
        ...
        public void EnableControls()
        {
            ...
            Cmb_To.Enabled = true;
            Cmb_From2.Enabled = true;
            Cmb_To2.Enabled = true;
        ...
        private void Addbtn2_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (TXT_TRNO.Text == "67")
                {
                    if (string.IsNullOrWhiteSpace(Cmb_From2.Text) || string.IsNullOrWhiteSpace(Cmb_To2.Text))
                    {
                        MessageBox.Show("يجب اختيار من و الى");
                        return;
                    }
        ...
        private void SaveBtn_Click(object sender, EventArgs e)
        {
            ...
            if (AddEditFlag == 2)
            {
                if (FlagSign11 != 1)
                {
                    MessageBox.Show("من فضلك تاكد من توقيع اذن التحويل");
                    return;
                }
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
        ...
        private void Getdata(string cmd)
        {
            ...
            dataGridView1.Columns["FNsbetSalhia"].Visible = false;
            ...
            */
            ...
            dataGridView1.Columns["TStockNoALL"].Visible = false;
            ...
            dataGridView1.Columns["TUnit"].Visible = false;
            ...
            dataGridView1.Columns["TBayan"].Visible = false;
           ...
           dataGridView1.Columns["TRased"].HeaderText = "79/72";//col16
            ...
            dataGridView1.Columns["TRased"].Visible = false;
        ...
        {
            if (FlagSign1 != 1 || FlagSign2 != 1 || FlagSign11 != 1)
            {
                MessageBox.Show("من فضلك تاكد من التوقيعات السابقة");
                return;
            }
        ...
        private void Cmb_CType_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (TXT_TRNO.Text == "66")
                {
                    ...
                    Cmb_From.SelectedIndexChanged += new EventHandler(Cmb_From_SelectedIndexChanged);
                    Label_From.Text = "من مخزن";
                    Label_To.Text = "الى مورد";
                  ...
                  /*  cmdstring = "SELECT  [CCode],[CName] FROM T_InventoryNames";//will use cmdstring3
                    ...
                    Cmb_To.SelectedIndexChanged += new EventHandler(Cmb_To_SelectedIndexChanged);*/
                    Cmb_To.Text = Constants.MwardName;
                }
                else if (TXT_TRNO.Text == "67")
                {
                    ...
                    Cmb_To.Text = Constants.MwardName;
                    Cmb_From.SelectedIndexChanged -= new EventHandler(Cmb_From_SelectedIndexChanged);
                    Label_From.Text = "من مورد";
                    Label_To.Text = "الى مخزن";
                    /* string cmdstring = "SELECT  [CCode],[CName] FROM T_InventoryNames";//will use cmdstring3
                    ...
                    dts2.Load(cmd.ExecuteReader());
                    Cmb_To2.DataSource = dts2;
                    Cmb_To2.ValueMember = "CCode";
                    Cmb_To2.DisplayMember = "CName";
                    Cmb_To2.SelectedIndex = -1;
                    Cmb_To2.SelectedIndexChanged += new EventHandler(Cmb_To_SelectedIndexChanged);
                }
            }
        }
        ...
        {
            Empn11 = Microsoft.VisualBasic.Interaction.InputBox("من فضلك ادخل رقم القيد الخاص بك", "توقيع على انشاء اذن تحويل", "");

            Sign11 = Microsoft.VisualBasic.Interaction.InputBox("من فضلك ادخل الرقم السرى الخاص بك", "توقيع على انشاء اذن تحويل", "");
            if (Sign11 != "" && Empn11!= "")
            {
                //  MessageBox.Show("done");
                // string result= Constants.CheckSign("1",Sign1);
                Tuple<string, int, int, string, string> result = Constants.CheckSign("11", "7", Sign11, Empn11);
                if (result.Item3 == 1)
                {
        ...
        private void Cmb_Msg_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            //    cmd = new SqlCommand(query, con);
                cmd.Parameters.AddWithValue("@FY", (Cmb_FyearMsg.Text));
        ...
        private void Cmb_FyearMsg_SelectedIndexChanged(object sender, EventArgs e)
        {
            Constants.opencon();
           ...
           // TXT_msg.AutoCompleteSource = AutoCompleteSource.None; ;
            ...
            string cmdstring3 = "select Edafa_No FROM [ANRPC_Inventory_foriegn_v2].[dbo].[T_Edafa]  where Edafa_FY='" + Cmb_FyearMsg.Text + "'" ;
            SqlCommand cmd3 = new SqlCommand(cmdstring3, Constants.con);
            SqlDataReader dr3 = cmd3.ExecuteReader();
            //---------------------------------
            if (dr3.HasRows == true)
        ...
        private void BTN_AddBnod_Click(object sender, EventArgs e)
        {
            if (string.IsNullOrEmpty(Cmb_FyearMsg.Text))
            {
                MessageBox.Show("من فضلك اختار السنة المالية ");
                return;
            }
          
            if (string.IsNullOrEmpty(Cmb_Msg.Text))
            {
                MessageBox.Show("من فضلك اختار رقم الرسالة");
                return;
            }
           


          //  GetData((Cmb_FyearMsg.Text), Cmb_FyearMsg.Text);

            EzonTahwel_PopUp popup = new EzonTahwel_PopUp();
            // popup.Show();

             popup.BM2 = Cmb_FyearMsg.Text;
             popup.BM= Cmb_Msg.Text ;
                ...
                if (popup.dataGridView1.SelectedRows.Count > 0)
                {
                    ...
                    foreach (DataGridViewRow row in popup.dataGridView1.Rows)
                    {
                        ...
                        if (!row.IsNewRow && row.Selected)
                        {
                            ...
                            dataGridView1.DataSource = table;
                            dataGridView1.Rows[r].Cells[3].Value = r+1;
                             dataGridView1.Rows[r].Cells[4].Value = row.Cells[7].Value;
                            if (TXT_TRNO.Text == "66")
                            {
                                dataGridView1.Rows[r].Cells[5].Value = Cmb_From.Text;
                                dataGridView1.Rows[r].Cells[12].Value = Cmb_To.Text;
                            ...
                            {
                                dataGridView1.Rows[r].Cells[5].Value = Cmb_From2.Text;
                                dataGridView1.Rows[r].Cells[12].Value = Cmb_To2.Text;
                            }
                  
                           dataGridView1.Rows[r].Cells[6].Value = row.Cells[8].Value;//rakmtasnif

                            dataGridView1.Rows[r].Cells[7].Value = row.Cells[9].Value;//whda
                            dataGridView1.Rows[r].Cells[8].Value = row.Cells[10].Value;//byan
                            dataGridView1.Rows[r].Cells[9].Value = row.Cells[11].Value;//rased
                            dataGridView1.Rows[r].Cells[10].Value = row.Cells[12].Value;//kyma
